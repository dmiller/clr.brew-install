...
(ns clojure.run.exec
  ...
  (:import
    ...
    #?(:clj [java.io StringWriter Writer FileNotFoundException]
...
(defn- envelope
  ...
  (let [end (get-current-system-time)]
    (cond-> {:tag tag
...
:cljr
;; not sure what the correct equivalent is.  CLR does not have "system properties".   Could try to pick this up from a config asset instead of an env var.
(defn- read-basis
  []
  (when-let [f (cio/file-info (Environment/GetEnvironmentVariable "clojure.basis"))]
    (if (and f (.Exists f))
     (->> f slurp (edn/read-string {:default tagged-literal}))
     (throw (err "No basis declared in clojure.basis system property")))))
)	 
...
)  ;;; doesn't do anything (at this time) in ClojureCLR
