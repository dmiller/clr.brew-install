(ns clojure.run.test-exec
  ...
  (:import
    #?(:clj [java.io File]
	   :cljr [System.IO Path FileStream StreamWriter FileInfo FileMode FileAccess FileShare FileOptions])
...
(defn- encapsulate-main-with-effect
  [exec-args args]
  (let [fake-fs (FileStream. (Path/GetTempFileName) FileMode/Open)
        fake-basis (StreamWriter. fake-fs)]
    (save nil)
    (binding [*print-namespace-maps* false]
      (spit fake-basis (pr-str {:argmap exec-args}))
      (Environment/SetEnvironmentVariable "clojure.basis" (.Name fake-fs)))
    (binding [exec/*exit* (constantly nil)]
      (apply exec/-main args))
    @stash))
)
...
(defn puppet [{:keys [ret err print-val print-val-err]}]
  ...
  (when print-val-err
    (binding [*out* *err*]
      ...
      (flush)))
  (if err
    (throw (#?(:clj RuntimeException. :cljr Exception.) err))
    ret))
...
(defmacro with-err-str
  ...
  {:added "1.0"}
  [& body]
  `(let [s# (new #?(:clj java.io.StringWriter :cljr System.IO.StringWriter))]
     (binding [*err* s#]
       ~@body
       (str s#))))
...
(deftest test-main
  ...
  (let [res (encapsulate-invoke {} (map pr-str ['clojure.run.test-exec/puppet {:clojure.exec/invoke :fn, :err "msg"}]))]
    (is (= :err (:tag res)))
    (is (str/starts-with? (:val res) #?(:clj "{:via [{:type java.lang.RuntimeException, :message \"msg\""
	                                    :cljr "{:via [{:type System.Exception, :message \"msg\""))))
